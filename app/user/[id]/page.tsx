...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  return {
    title: `${profile?.display_name || "User"} - RefZone`,
    description: `View ${profile?.display_name || "user"}'s profile and posts`,
  }
}
...
export default async function UserProfilePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (isUnder16(currentProfile?.date_of_birth)) {
      return (
        <div className="p-6 lg:p-8 space-y-6 max-w-4xl mx-auto">
          ...
          <AgeRestrictionBanner feature="user profiles" />
        </div>
      )
    }
  }
  ...
  const { data: customization } = await supabase
    ...
    .select(`
      ...
      theme:shop_items!user_customization_active_theme_id_fkey(name, preview_data)
    `)
    .eq("user_id", id)
    .maybeSingle()

  // Apply theme if exists
  const theme = (customization as any)?.theme
  const themeStyles = theme?.preview_data
    ? {
        background: `linear-gradient(135deg, ${theme.preview_data.primaryColor || "#6366F1"}, ${theme.preview_data.secondaryColor || "#8B5CF6"})`,
      }
    : {}
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      ...
      >
        <CardContent
          className="pt-6"
          style={Object.keys(themeStyles).length > 0 ? { backgroundColor: "transparent" } : undefined}
        >
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
            <div
              className={cn(
                "flex h-20 w-20 items-center justify-center rounded-full text-3xl font-bold",
                Object.keys(themeStyles).length > 0 ? "bg-white/20 text-white" : "bg-primary text-primary-foreground",
              )}
            >
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3">
                <h1 className={cn("text-2xl font-bold", Object.keys(themeStyles).length > 0 && "text-white")}>
              ...
              <div
                className={cn(
                  "flex flex-wrap gap-4 mt-2 text-sm",
                  Object.keys(themeStyles).length > 0 ? "text-white/90" : "text-muted-foreground",
                )}
              >
